-- Lighting Hub v4 - FINAL [AutoSteal build]
-- Original by OR, modified to Lighting Hub --

-- ============================================================
-- RESOLVE fireproximityprompt (compativel com key system / loadstring)
-- ============================================================
...
-- ============================================================
-- SISTEMA DE CONFIGURACAO UNIFICADO
-- ============================================================
local GLOBAL_CONFIG_FILE = "LightingHub_Config.json"
local _globalConfig = {}
...
local _DEFAULT_CONFIG = {
    ...
    duelPositions   = {
        P1x=-474.8211975097656,  P1y=-6.992471694946289,  P1z=93.64490509033203,
        P2x=-479.4603271484375,  P2y=-5.106024742126465,  P2z=93.8957290649414,
        PEXx=-484.7935791015625, PEXy=-4.877744197845459, PEXz=96.35384368896485,
        R1x=-474.8984375,        R1y=-6.992471694946289,  R1z=26.0786075592041,
        R2x=-479.8058776855469,  R2y=-5.0522565841674809, R2z=25.93069076538086,
        REXx=-484.89208984375,   REXy=-4.877744197845459, REXz=24.079252243041993,
        pEndDelay=0.028, rEndDelay=0.028,
    },
    togglesState    = {
        speedCustomizer=false, autoTP=false,  noAnim=false,
        lock=true,  autoDuel=true,  hitTP=false,   meleeAimbot=false,
...
pcall(function()
    if CoreGui:FindFirstChild("LightingHub_v4") then
        CoreGui:FindFirstChild("LightingHub_v4"):Destroy()
    end
end)
...
local _introFinished = Instance.new("BindableEvent")

local Screen = Instance.new("ScreenGui")
Screen.Name           = "LightingHub_v4"
Screen.ResetOnSpawn   = false
Screen.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Screen.Enabled        = false
Screen.Parent         = CoreGui
_introFinished.Event:Connect(function()
    Screen.Enabled = true
    for _, d in ipairs(Screen:GetDescendants()) do
        if (d:IsA("Frame") or d:IsA("CanvasGroup")) and d.BackgroundTransparency < 0.99 then
            local orig = d.BackgroundTransparency
            d.BackgroundTransparency = 1
            TweenService:Create(d, TweenInfo.new(1.0, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {BackgroundTransparency = orig}):Play()
        end
        if d:IsA("TextLabel") or d:IsA("TextButton") then
            local orig = d.TextTransparency
            d.TextTransparency = 1
            TweenService:Create(d, TweenInfo.new(1.0, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {TextTransparency = orig}):Play()
        end
        if d:IsA("UIStroke") then
            d.Transparency = 1
            TweenService:Create(d, TweenInfo.new(1.0, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Transparency = 0}):Play()
        end
        if d:IsA("ImageLabel") or d:IsA("ImageButton") then
            d.ImageTransparency = 1
            TweenService:Create(d, TweenInfo.new(1.0, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {ImageTransparency = 0}):Play()
        end
    end
end)

-- ============================================================
-- INTRO: "Lighting Hub > Nine Duels"
-- ============================================================
task.spawn(function()
    local introGui = Instance.new("ScreenGui", CoreGui)
    introGui.Name = "LightingHubIntro"
    ...
    task.spawn(function()
        pcall(function()
            local s = Instance.new("Sound", workspace)
            s.SoundId = "rbxassetid://861942173"
            s.Volume = 0.7
            s:Play()
...
-- ============================================================
local C = {
    Black     = Color3.fromRGB(35, 8, 8),
    DarkRow   = Color3.fromRGB(28, 6, 6),
    DarkTab   = Color3.fromRGB(22, 5, 5),
    ...
    TogOff    = Color3.fromRGB(40, 45, 55),
}

local STROKE_ACTIVE = Color3.fromRGB(200, 30, 30)
local STROKE_IDLE   = Color3.fromRGB(15, 15, 15)
local DOT_ON  = Color3.fromRGB(220, 40, 40)
local DOT_OFF = Color3.fromRGB(80, 30, 30)
    ...
    task.spawn(function()
        while stroke and stroke.Parent do
            TweenService:Create(stroke,
                ...
                {Color = Color3.fromRGB(180, 25, 25)}):Play()
            task.wait(1.4)
            TweenService:Create(stroke,
                TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
                {Color = Color3.fromRGB(15, 15, 15)}):Play()
            task.wait(1.4)
        end
    end)
    return stroke
...
local function applyHubGradient(frame)
    local og = frame:FindFirstChildOfClass("UIGradient")
    if og then og:Destroy() end
    frame.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
...
local function setHubActive(hub, dot, active)
    ...
    stroke.Color = active and STROKE_ACTIVE or STROKE_IDLE
    if dot then
        dot.Text = "â—‰"
...
miniFrame.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
...
mStroke.Thickness = 2
mStroke.Color = Color3.fromRGB(15, 15, 15)
task.spawn(function()
    while mStroke and mStroke.Parent do
        TweenService:Create(mStroke, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
...
hStroke.Thickness = 2; hStroke.Color = Color3.fromRGB(15, 15, 15)
task.spawn(function()
    while hStroke and hStroke.Parent do
        TweenService:Create(hStroke,
            TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
...
local hubTitleLbl = New("TextLabel", {
    Text = "LIGHTING HUB", TextColor3 = Color3.fromRGB(230, 50, 50),
...
local TAB_ROWS = {Combat={}, Protect={}, Visual={}, Settings={}}
local Pages    = {}
local TabBtns  = {}
...
local function MakePage(name)
    ...
    b.MouseButton1Click:Connect(function()
        ...
        sf.Visible=true; b.TextColor3=Color3.fromRGB(200, 30, 30)
    end)
    TabBtns[name] = b
end

for _, t in ipairs(TABS) do MakePage(t) end
TabBtns["Combat"].TextColor3 = Color3.fromRGB(200, 30, 30)
...
local function Toggle(page, label, default, cb)
    ...
    local bg = New("Frame", {
        ...
        BackgroundColor3=on and Color3.fromRGB(200, 30, 30) or C.TogOff, ZIndex=8, Parent=row,
    })
    ...
    New("TextButton", {
        ...
        on = not on
        TweenService:Create(bg,  TweenInfo.new(0.18), {BackgroundColor3=on and Color3.fromRGB(200, 30, 30) or C.TogOff}):Play()
...
-- BARRA GRAB / DUEL
-- ============================================================
local grabBarFill, grabBarPercent, gbStroke
local radBox         = nil
local durBox         = nil
...
infoBarGui.Enabled = false
_introFinished.Event:Connect(function()
    infoBarGui.Enabled = true
    for _, d in ipairs(infoBarGui:GetDescendants()) do
        if (d:IsA("Frame") or d:IsA("CanvasGroup")) and d.BackgroundTransparency < 0.99 then
            local orig = d.BackgroundTransparency
            d.BackgroundTransparency = 1
            TweenService:Create(d, TweenInfo.new(1.0, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {BackgroundTransparency = orig}):Play()
    ...
    task.delay(1.5, function()
        for _, d in ipairs(infoBarGui:GetDescendants()) do
            pcall(function()
                ...
                if d:IsA("UIStroke") then d.Transparency = 0 end
            end)
        end
    end)
end)
...
topFrame.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
...
topStroke.Color = Color3.fromRGB(15, 15, 15)
task.spawn(function()
    while topStroke and topStroke.Parent do
        TweenService:Create(topStroke, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
            {Color = Color3.fromRGB(200, 30, 30)}):Play()
        task.wait(1.4)
        TweenService:Create(topStroke, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
            {Color = Color3.fromRGB(15, 15, 15)}):Play()
...
botFrame.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
...
botStroke.Color = Color3.fromRGB(15, 15, 15)
gbStroke = botStroke
task.spawn(function()
    while botStroke and botStroke.Parent do
        TweenService:Create(botStroke, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
            {Color = Color3.fromRGB(200, 30, 30)}):Play()
        task.wait(1.4)
        TweenService:Create(botStroke, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
            {Color = Color3.fromRGB(15, 15, 15)}):Play()
...
-- ============================================================
-- SPEED ACIMA DA CABECA (BillboardGui)
-- ============================================================
do
...
local _holdKB = {key=nil, waiting=false}
...
local function startSpeedWhileStealing()
    ...
    speedWhileStealConn = RunService.Heartbeat:Connect(function()
        ...
        if md.Magnitude > 0.1 then
            local tx = md.X * stealingSpeedValue
            local tz = md.Z * stealingSpeedValue
            local cv = h.AssemblyLinearVelocity
            local nx = (md.X * cv.X < 0 or math.abs(cv.X) < math.abs(tx)) and tx or cv.X
            local nz = (md.Z * cv.Z < 0 or math.abs(cv.Z) < math.abs(tz)) and tz or cv.Z
            h.AssemblyLinearVelocity = Vector3.new(nx, cv.Y, nz)
        end
    end)
end
...
-- ============================================================
-- AUTO DUEL HUB (createDuelHub) - sem TP stuff
-- ============================================================
local duelGui    = nil
dbFrame          = nil

-- ============================================================
-- AUTO STEAL (sistema K7)
-- ============================================================
...
local function createDuelHub()
    ...
    local P1      = Vector3.new(_pd.P1x   or -474.8211975097656,  _pd.P1y   or -6.992471694946289,  _pd.P1z   or 93.64490509033203)
    local P2      = Vector3.new(_pd.P2x   or -479.4603271484375,  _pd.P2y   or -5.106024742126465,  _pd.P2z   or 93.8957290649414)
    local P_EXTRA = Vector3.new(_pd.PEXx  or -484.7935791015625,  _pd.PEXy  or -4.877744197845459,  _pd.PEXz  or 96.35384368896485)
    ...
    local R1      = Vector3.new(_pd.R1x   or -474.8984375,        _pd.R1y   or -6.992471694946289,  _pd.R1z   or 26.0786075592041)
    local R2      = Vector3.new(_pd.R2x   or -479.8058776855469,  _pd.R2y   or -5.0522565841674809, _pd.R2z   or 25.93069076538086)
    local R_EXTRA = Vector3.new(_pd.REXx  or -484.89208984375,    _pd.REXy  or -4.877744197845459,  _pd.REXz  or 24.079252243041993)
    local pEndDelay = _pd.pEndDelay or 0.028
    local rEndDelay = _pd.rEndDelay or 0.028
    ...
    fAuto.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
    ...
    lblAuto.Text = "â—‰"; lblAuto.ZIndex = 12
    ...
    local function atualizarBtnAutoVisual()
        ...
        elseif AutoPlayAtivo then
            btnAuto.TextColor3 = Color3.fromRGB(255,255,255)
            autoStroke.Color = STROKE_ACTIVE
        ...
        heartbeatL = RunService.Heartbeat:Connect(function()
            ...
            if dist < 1.5 then
                ...
                elseif phase == 2 then
                    ...
                    heartbeatL:Disconnect(); heartbeatL = nil
                    if rEndDelay and rEndDelay > 0 then
                        task.delay(rEndDelay, function() returnToBaseLeft() end)
                    else
                        returnToBaseLeft()
                    end
                    return
        ...
        heartbeatR = RunService.Heartbeat:Connect(function()
            ...
            if dist < 1.5 then
                ...
                elseif phase == 2 then
                    ...
                    if mvSpeedOn and _bridgeSetMvSpeed then _bridgeSetMvSpeed(duelReturnSpeed) end
                    heartbeatR:Disconnect(); heartbeatR = nil
                    if pEndDelay and pEndDelay > 0 then
                        task.delay(pEndDelay, function() returnToBaseRight() end)
                    else
                        returnToBaseRight()
                    end
                    return
    ...
    local function iniciarAutoPlay()
        ...
        if aimEnabled and stopAimBot then
            aimEnabled = false; stopAimBot()
            if LockFrame and aimRedDot then setHubActive(LockFrame, aimRedDot, false) end
        end
        if abEnabled and stopAbAim then
            abEnabled = false; stopAbAim()
            if abFrame and abRedDot then setHubActive(abFrame, abRedDot, false) end
        end
    ...
    do
        ...
        local function stopBrLook()
            if _brConn  then _brConn:Disconnect();  _brConn  = nil end
            if _brAlign then pcall(function() _brAlign:Destroy() end); _brAlign = nil end
            if _brAttch then pcall(function() _brAttch:Destroy() end); _brAttch = nil end
        ...
        local function startBrLook()
            stopBrLook()
            local c = lp.Character; if not c then return end
            local hrp = c:FindFirstChild("HumanoidRootPart"); if not hrp then return end
            _brAttch = Instance.new("Attachment", hrp)
            _brAlign = Instance.new("AlignOrientation", hrp)
            _brAlign.Attachment0     = _brAttch
            _brAlign.Mode            = Enum.OrientationAlignmentMode.OneAttachment
            _brAlign.RigidityEnabled = true
            _brAlign.Enabled         = false
            _brConn = RunService.Heartbeat:Connect(function()
                if not hasBrainrotInHand() then stopBrLook(); return end
                ...
                local nearest, nearDist = nil, 20
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= lp and p.Character then
                        local eh = p.Character:FindFirstChild("HumanoidRootPart")
                        if eh then
                            local d = (eh.Position - h.Position).Magnitude
                            if d < nearDist then nearest, nearDist = eh, d end
                        end
                    end
                end
                if nearest and _brAlign then
                    local diff = nearest.Position - h.Position
                    local flat = Vector3.new(diff.X, 0, diff.Z)
                    if flat.Magnitude > 0.1 then
                        hum.AutoRotate = false
                        _brAlign.Enabled = true
                        _brAlign.CFrame  = CFrame.lookAt(Vector3.zero, flat)
                    end
                else
                    if _brAlign then _brAlign.Enabled = false end
                    hum.AutoRotate = true
                end
            end)
        end

        local function checkBR()
            if hasBrainrotInHand() then
                if not _brConn then startBrLook() end
            else
                stopBrLook()
            end
        end
        ...
        local function connectChar(char)
            ...
            char.ChildAdded:Connect(function(o)
                if o:IsA("Tool") then task.wait(0.05); checkBR() end
            end)
            char.ChildRemoved:Connect(function(o)
                if o:IsA("Tool") then task.wait(0.05); checkBR() end
            end)
            pcall(function()
                lp:GetAttributeChangedSignal("Stealing"):Connect(checkBR)
            end)
        end

        connectChar(lp.Character)
        lp.CharacterAdded:Connect(function(char)
            task.wait(0.5); stopBrLook(); connectChar(char)
        end)
    end
    ...
    return {
        ...
        setLado = function(lado)
            ladoEscolhido = lado
        end,
        setSideVisual = function(lado)
            ladoEscolhido = lado
            atualizarPainelVisual()
            atualizarBtnAutoVisual()
        end,
        ...
        getDelays = function()
            return {pEndDelay=pEndDelay, rEndDelay=rEndDelay}
        end,
        setDelay = function(name, val)
            if name == "pEndDelay" then pEndDelay = val
            elseif name == "rEndDelay" then rEndDelay = val
            end
            local _pd2 = cfgGet("duelPositions") or {}
            _pd2.pEndDelay = pEndDelay
            _pd2.rEndDelay = rEndDelay
            cfgSet("duelPositions", _pd2)
        end,
...
;(function()
    ...
    fPos.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
    fPos.Active = true
    ...
    local DELAY_ROW_H = 26
    local H_DELAY     = DELAY_ROW_H * 2 + 4
    fPos.Size = UDim2.new(0, W, 0, H_TOTAL + H_DELAY)

    local delayDefs = {
        {key="pEndDelay", label="P End Delay", color=Color3.fromRGB(30,80,180)},
        {key="rEndDelay", label="R End Delay", color=Color3.fromRGB(180,40,30)},
    }
    local delayBaseY = H_TITLE + #KEYS * ROW_H + 4

    for di, dd in ipairs(delayDefs) do
        local rowY = delayBaseY + (di-1) * DELAY_ROW_H

        local lbl = Instance.new("TextLabel", fPos)
        lbl.Size = UDim2.new(0, 88, 0, DELAY_ROW_H)
        lbl.Position = UDim2.new(0, PAD, 0, rowY)
        lbl.BackgroundTransparency = 1
        lbl.Text = dd.label
        lbl.Font = Enum.Font.Bangers
        lbl.TextSize = 13
        lbl.TextColor3 = dd.color
        lbl.TextXAlignment = Enum.TextXAlignment.Left

        local btnMinus = Instance.new("TextButton", fPos)
        btnMinus.Size = UDim2.new(0, 22, 0, 18)
        btnMinus.Position = UDim2.new(0, 100, 0, rowY + 4)
        btnMinus.BackgroundColor3 = Color3.fromRGB(25,25,25)
        btnMinus.Font = Enum.Font.GothamBold
        btnMinus.TextSize = 14
        btnMinus.TextColor3 = Color3.fromRGB(200,200,200)
        btnMinus.Text = "-"
        btnMinus.AutoButtonColor = false
        btnMinus.BorderSizePixel = 0
        Instance.new("UICorner", btnMinus).CornerRadius = UDim.new(0,5)
        local stk1 = Instance.new("UIStroke", btnMinus); stk1.Color = dd.color; stk1.Thickness = 1

        local valLbl = Instance.new("TextLabel", fPos)
        valLbl.Size = UDim2.new(0, 36, 0, DELAY_ROW_H)
        valLbl.Position = UDim2.new(0, 124, 0, rowY)
        ...
        local btnPlus = Instance.new("TextButton", fPos)
        btnPlus.Size = UDim2.new(0, 22, 0, 18)
        btnPlus.Position = UDim2.new(0, 162, 0, rowY + 4)
        btnPlus.BackgroundColor3 = Color3.fromRGB(25,25,25)
        btnPlus.Font = Enum.Font.GothamBold
        btnPlus.TextSize = 14
        btnPlus.TextColor3 = Color3.fromRGB(200,200,200)
        btnPlus.Text = "+"
        btnPlus.AutoButtonColor = false
        btnPlus.BorderSizePixel = 0
        Instance.new("UICorner", btnPlus).CornerRadius = UDim.new(0,5)
        local stk2 = Instance.new("UIStroke", btnPlus); stk2.Color = dd.color; stk2.Thickness = 1

        local function refreshVal()
            if duelModule and duelModule.getDelays then
                local d = duelModule.getDelays()
                valLbl.Text = string.format("%.1fs", d[dd.key] or 0)
            else
                valLbl.Text = "0.0s"
            end
        end
        task.defer(refreshVal)

        local function changeDelay(delta)
            if not duelModule or not duelModule.setDelay then return end
            local d = duelModule.getDelays()
            local cur = d[dd.key] or 0
            cur = math.max(0, math.floor((cur + delta) * 10 + 0.5) / 10)
            duelModule.setDelay(dd.key, cur)
            refreshVal()
        end

        btnMinus.MouseButton1Click:Connect(function() changeDelay(-0.1) end)
        btnPlus.MouseButton1Click:Connect(function()  changeDelay( 0.1) end)
    end

    local showBtn = Instance.new("TextButton", fPos)
    showBtn.Size = UDim2.new(1, -16, 0, BTN_H - 4)
    showBtn.Position = UDim2.new(0, 8, 0, H_TITLE + #KEYS * ROW_H + H_DELAY + PAD)
...
startAimBot = function()
    ...
    aimConnections.aimbot = RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            local enemyChar = target.Parent
            if enemyChar and not enemyChar:FindFirstChild("_AimHitbox") then
                local hb = Instance.new("Part")
                hb.Name = "_AimHitbox"; hb.Size = Vector3.new(6,6,6)
                hb.Transparency = 1; hb.CanCollide = false; hb.Massless = true
                hb.CFrame = target.CFrame; hb.Parent = enemyChar
                local w = Instance.new("WeldConstraint")
                w.Part0 = target; w.Part1 = hb; w.Parent = hb
            end
        end)
        ...
        elseif flatDist > 0.8 then
            local frac = (flatDist - 0.8) / 1.7
            local spd  = AimSpeed * math.max(frac, 0.1)
            local moveDir = flatDir.Unit
            h.AssemblyLinearVelocity = Vector3.new(
                moveDir.X * spd, velY, moveDir.Z * spd)
        else
            local tv = target.AssemblyLinearVelocity
            h.AssemblyLinearVelocity = Vector3.new(tv.X * 0.3, velY, tv.Z * 0.3)
            local snapPos = target.Position + (h.Position - target.Position).Unit * 0.7
            h.CFrame = CFrame.new(Vector3.new(snapPos.X, h.Position.Y, snapPos.Z))
        end
    end)
end

local function cleanAimHitboxes()
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= lp and p.Character then
            local hb = p.Character:FindFirstChild("_AimHitbox")
...
LockFrame.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
...
aimRedDot.Text = "â—‰"; aimRedDot.TextColor3 = DOT_OFF
...
do
    ...
    local function startCebo()
        ...
        Cebo.Conn = RunService.RenderStepped:Connect(function()
            ...
            if tgt then
                ...
                if t and (not Cebo._lastSwing or tick() - Cebo._lastSwing >= 0.22) then
                    Cebo._lastSwing = tick()
    ...
    maFrame.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
    ...
    maRedDot.Text = "â—‰"; maRedDot.TextColor3 = DOT_OFF
...
do
    ...
    startAbAim = function()
        ...
        abConnections.main = RunService.Heartbeat:Connect(function()
            ...
            local target = findNearestEnemyAim(h)
            if not target or not target.Parent then
            ...
            if _abLastTarget ~= target then
                abConnections._lastSwing = nil
                _abLastTarget = target
            end
            ...
            pcall(function()
                local ec = target.Parent
                if ec and not ec:FindFirstChild("_AimHitbox") then
                    local hb = Instance.new("Part")
                    hb.Name = "_AimHitbox"; hb.Size = Vector3.new(6,6,6)
                    hb.Transparency = 1; hb.CanCollide = false; hb.Massless = true
                    hb.CFrame = target.CFrame; hb.Parent = ec
                    local w = Instance.new("WeldConstraint")
                    w.Part0 = target; w.Part1 = hb; w.Parent = hb
                end
            end)

            local abSpeedVal = (_abDadosRef and _abDadosRef.speed) or AimSpeed
            local velY = math.clamp(diff.Y * 18, -abSpeedVal, abSpeedVal)
            ...
            elseif flatDist > 0.8 then
                local frac = (flatDist - 0.8) / 1.7
                local spd  = abSpeedVal * math.max(frac, 0.1)
                local moveDir = flatDir.Unit
                h.AssemblyLinearVelocity = Vector3.new(
                    moveDir.X * spd, velY, moveDir.Z * spd)
            else
                ...
                if t and t.Parent == c then
                    if not abConnections._lastSwing or tick() - abConnections._lastSwing >= 0.22 then
                        abConnections._lastSwing = tick()
                        pcall(function() t:Activate() end)
                    end
                end
            end
    ...
    abFrame.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
    ...
    abRedDot.Text = "â—‰"; abRedDot.TextColor3 = DOT_OFF
    ...
    abGear.Text = "âš™"; abGear.BackgroundTransparency = 1
...
;(function()
    ...
    FloatFrame.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
    ...
    floatRedDot.Text = "â—‰"; floatRedDot.TextColor3 = DOT_OFF
...
-- ============================================================
-- SPEED HUB (Speed Customizer) - sem spin
-- ============================================================
...
local function createSpeedHub()
    ...
    fSpeed.BackgroundColor3 = Color3.fromRGB(35, 8, 8); fSpeed.Active = true
    ...
    local function atualizarVisualSpeed(v)
        ...
        speedTogBtn.BackgroundColor3 = v and Color3.fromRGB(200, 30, 30) or C.TogOff
    end
    ...
    local function _doStartMvSpeed()
        ...
        mvConn = RunService.Heartbeat:Connect(function()
            ...
            if md.Magnitude > 0.1 then
                local targetVelX = md.X * currentSpeed
                local targetVelZ = md.Z * currentSpeed
                local curVel = h.AssemblyLinearVelocity
                local newX = math.abs(curVel.X) < math.abs(targetVelX) and targetVelX or curVel.X
                local newZ = math.abs(curVel.Z) < math.abs(targetVelZ) and targetVelZ or curVel.Z
                if md.X * curVel.X < 0 then newX = targetVelX end
                if md.Z * curVel.Z < 0 then newZ = targetVelZ end
                h.AssemblyLinearVelocity = Vector3.new(newX, curVel.Y, newZ)
            end
        end)
    end
    ...
    mvTogBtn.BackgroundColor3 = mvOn and Color3.fromRGB(200, 30, 30) or C.TogOff
    ...
    mvTogBtn.MouseButton1Click:Connect(function()
        ...
        mvTogBtn.BackgroundColor3 = mvSpeedOn and Color3.fromRGB(200, 30, 30) or C.TogOff
...
-- HOLD AUTO DUEL
task.spawn(function()
    local holdDados = cfgGet("holdDuelPos") or {px=-1, py=-1}

    local fHold = Instance.new("Frame", Screen)
    fHold.Name = "HoldDuelFrame"
    fHold.Size = UDim2.new(0, 128, 0, 34)
    fHold.Active = true; fHold.ZIndex = 10; fHold.Visible = false
    fHold.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
    fHold.BorderSizePixel = 0; fHold.ClipsDescendants = true
    if holdDados.px and holdDados.px >= 0 then
        fHold.Position = UDim2.new(0, holdDados.px, 0, holdDados.py)
    else
        fHold.Position = UDim2.new(0.5, -64, 0.22, 0)
    end
    Instance.new("UICorner", fHold).CornerRadius = UDim.new(0, 10)
    applyHubGradient(fHold)

    local holdStroke = Instance.new("UIStroke", fHold)
    holdStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    holdStroke.Thickness = 2; holdStroke.Color = STROKE_IDLE

    local holdDot = Instance.new("TextLabel", fHold)
    holdDot.BackgroundTransparency = 1
    holdDot.Size = UDim2.new(0,14,0,14); holdDot.Position = UDim2.new(0,3,0,10)
    holdDot.Text = "â—‰"; holdDot.ZIndex = 12
    holdDot.TextColor3 = DOT_OFF
    holdDot.Font = Enum.Font.GothamBold; holdDot.TextSize = 13

    local holdBtn = Instance.new("TextButton", fHold)
    holdBtn.BackgroundTransparency = 1; holdBtn.BorderSizePixel = 0
    holdBtn.Font = Enum.Font.Bangers; holdBtn.TextSize = 17
    holdBtn.Size = UDim2.new(1,-28,0,32); holdBtn.Position = UDim2.new(0,5,0,1)
    holdBtn.Text = "HOLD DUEL"
    holdBtn.TextXAlignment = Enum.TextXAlignment.Center
    holdBtn.AutoButtonColor = false; holdBtn.ZIndex = 12
    holdBtn.TextColor3 = Color3.fromRGB(255,255,255)

    local holdGear = Instance.new("TextButton", fHold)
    holdGear.BackgroundTransparency = 1; holdGear.BorderSizePixel = 0
    holdGear.Font = Enum.Font.GothamBold; holdGear.TextSize = 12
    holdGear.TextColor3 = Color3.fromRGB(160,160,160)
    holdGear.Size = UDim2.new(0,22,0,22); holdGear.Position = UDim2.new(1,-24,0,6)
    holdGear.Text = "âš™"; holdGear.AutoButtonColor = false; holdGear.ZIndex = 13

    local holdDivider = Instance.new("Frame", fHold)
    holdDivider.Size = UDim2.new(1,-12,0,1); holdDivider.Position = UDim2.new(0,6,0,34)
    holdDivider.BackgroundColor3 = Color3.fromRGB(80,80,80)
    holdDivider.BorderSizePixel = 0; holdDivider.ZIndex = 11

    local lblSide = Instance.new("TextLabel", fHold)
    lblSide.BackgroundTransparency = 1; lblSide.TextColor3 = Color3.fromRGB(255,255,255)
    lblSide.Size = UDim2.new(1,0,0,18); lblSide.Position = UDim2.new(0,0,0,37)
    lblSide.Text = "CHOOSE SIDE"; lblSide.Font = Enum.Font.GothamBold
    lblSide.TextSize = 9; lblSide.ZIndex = 31

    local btnHR = Instance.new("TextButton", fHold)
    btnHR.BackgroundTransparency = 1; btnHR.BorderSizePixel = 0
    btnHR.Font = Enum.Font.Bangers; btnHR.TextSize = 15
    btnHR.Size = UDim2.new(0,52,0,24); btnHR.Position = UDim2.new(0,8,0,56)
    btnHR.Text = "RIGHT"; btnHR.AutoButtonColor = false; btnHR.ZIndex = 32
    btnHR.TextColor3 = Color3.fromRGB(255,255,255)
    Instance.new("UICorner", btnHR).CornerRadius = UDim.new(0,7)
    local sHR = Instance.new("UIStroke", btnHR)
    sHR.Thickness = 1.5; sHR.Color = STROKE_ACTIVE
    sHR.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; sHR.Enabled = false

    local btnHL = Instance.new("TextButton", fHold)
    btnHL.BackgroundTransparency = 1; btnHL.BorderSizePixel = 0
    btnHL.Font = Enum.Font.Bangers; btnHL.TextSize = 15
    btnHL.Size = UDim2.new(0,52,0,24); btnHL.Position = UDim2.new(0,66,0,56)
    btnHL.Text = "LEFT"; btnHL.AutoButtonColor = false; btnHL.ZIndex = 32
    btnHL.TextColor3 = Color3.fromRGB(255,255,255)
    Instance.new("UICorner", btnHL).CornerRadius = UDim.new(0,7)
    local sHL = Instance.new("UIStroke", btnHL)
    sHL.Thickness = 1.5; sHL.Color = STROKE_ACTIVE
    sHL.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; sHL.Enabled = false

    local holdDotRow = Instance.new("Frame", fHold)
    holdDotRow.BackgroundTransparency = 1
    holdDotRow.Size = UDim2.new(1,0,0,12); holdDotRow.Position = UDim2.new(0,0,0,84)
    holdDotRow.ZIndex = 31

    local dotHR = Instance.new("Frame", holdDotRow)
    dotHR.Size = UDim2.new(0,7,0,7); dotHR.Position = UDim2.new(0.5,-10,0.5,-3)
    dotHR.BackgroundColor3 = DOT_OFF; dotHR.BorderSizePixel = 0; dotHR.ZIndex = 32
    Instance.new("UICorner", dotHR).CornerRadius = UDim.new(1,0)

    local dotHL = Instance.new("Frame", holdDotRow)
    dotHL.Size = UDim2.new(0,7,0,7); dotHL.Position = UDim2.new(0.5,3,0.5,-3)
    dotHL.BackgroundColor3 = DOT_OFF; dotHL.BorderSizePixel = 0; dotHL.ZIndex = 32
    Instance.new("UICorner", dotHL).CornerRadius = UDim.new(1,0)

    local holdLadoEscolhido = nil

    local function atualizarPainelHold()
        sHR.Enabled = (holdLadoEscolhido == "RIGHT")
        sHL.Enabled = (holdLadoEscolhido == "LEFT")
        dotHR.BackgroundColor3 = (holdLadoEscolhido == "RIGHT") and DOT_ON or DOT_OFF
        dotHL.BackgroundColor3 = (holdLadoEscolhido == "LEFT")  and DOT_ON or DOT_OFF
    end
    atualizarPainelHold()

    local _holdExpand = makeInlineExpand(fHold, 34, 98)
    holdGear.MouseButton1Click:Connect(function()
        if _dragJustHappened then return end
        _holdExpand()
    end)

    btnHR.MouseButton1Click:Connect(function()
        if _dragJustHappened then return end
        holdLadoEscolhido = "RIGHT"
        atualizarPainelHold()
        holdDados.lado = "RIGHT"; cfgSet("holdDuelPos", holdDados)
        if duelModule and duelModule.setLado then duelModule.setLado("RIGHT") end
        if duelModule and duelModule.setSideVisual then duelModule.setSideVisual("RIGHT") end

        TweenService:Create(fHold, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            {Size = UDim2.new(0, 128, 0, 34)}):Play()
    end)

    btnHL.MouseButton1Click:Connect(function()
        if _dragJustHappened then return end
        holdLadoEscolhido = "LEFT"
        atualizarPainelHold()
        holdDados.lado = "LEFT"; cfgSet("holdDuelPos", holdDados)
        if duelModule and duelModule.setLado then duelModule.setLado("LEFT") end
        if duelModule and duelModule.setSideVisual then duelModule.setSideVisual("LEFT") end

        TweenService:Create(fHold, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            {Size = UDim2.new(0, 128, 0, 34)}):Play()
    end)

    MakeDrag(fHold, function()
        holdDados.px = fHold.AbsolutePosition.X
        holdDados.py = fHold.AbsolutePosition.Y
        cfgSet("holdDuelPos", holdDados)
    end)

    local _holdState = {input=nil, start=Vector2.zero, moved=0, thresh=10}

    holdBtn.InputBegan:Connect(function(inp)
        if inp.UserInputType ~= Enum.UserInputType.MouseButton1
        and inp.UserInputType ~= Enum.UserInputType.Touch then return end
        _holdState.input = inp; _holdState.moved = 0
        _holdState.start = Vector2.new(inp.Position.X, inp.Position.Y)
    end)

    UIS.InputChanged:Connect(function(inp)
        if inp ~= _holdState.input then return end
        _holdState.moved = (Vector2.new(inp.Position.X, inp.Position.Y) - _holdState.start).Magnitude
    end)
    ...
    UIS.InputEnded:Connect(function(inp)
        ...
        if inp.UserInputType ~= Enum.UserInputType.MouseButton1
        and inp.UserInputType ~= Enum.UserInputType.Touch then return end
        local moved = _holdState.moved; _holdState.input = nil; _holdState.moved = 0
        if moved >= _holdState.thresh then return end
        if not holdLadoEscolhido then return end
        if not duelModule then return end
        if duelModule.isActive() then
            duelModule.toggle()
        else
            if duelModule.setLado then duelModule.setLado(holdLadoEscolhido) end
            duelModule.toggle()
        end
        local active = duelModule.isActive()
        if active then
            if aimEnabled and stopAimBot then
                aimEnabled = false; stopAimBot()
                if LockFrame and aimRedDot then setHubActive(LockFrame, aimRedDot, false) end
        ...
        holdDot.TextColor3 = active and DOT_ON or DOT_OFF
        TweenService:Create(holdStroke, TweenInfo.new(0.18),
            {Color = active and STROKE_ACTIVE or STROKE_IDLE}):Play()
    end)

    RunService.Heartbeat:Connect(function()
        if not fHold.Visible then return end
        if not duelModule then return end
        local active = duelModule.isActive()
        holdDot.TextColor3 = active and DOT_ON or DOT_OFF
        holdStroke.Color = active and STROKE_ACTIVE or STROKE_IDLE
    end)

    UIS.InputBegan:Connect(function(inp, gp)
        if gp or _holdKB.waiting then return end
        if _holdKB.key and inp.KeyCode == _holdKB.key and fHold.Visible then
            if not holdLadoEscolhido then return end
            if not duelModule then return end
            if duelModule.isActive() then
                duelModule.toggle()
    ...
    do
        local saved = cfgGet("holdDuelPos") or {}
        if saved.keybind then
            local ok, kc = pcall(function() return Enum.KeyCode[saved.keybind] end)
            if ok and kc then _holdKB.key = kc end
        end
    end

    ToggleSaved("Combat", "Hold Auto Duel", "holdDuel", true, function(v)
        if v then hubOpen(fHold) else hubClose(fHold) end
    end)
end)
...
do
    ...
    dbFrame.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
    ...
    _dbRedDot.BackgroundTransparency = 1; _dbRedDot.Text = "â—‰"
...
ToggleSaved("Combat", "Hitbox Expander áµ’áµ–áµ—â±áµ’â¿áµƒË¡", "hitboxExp", true, function(v)
    hitboxActive = v
...
do
    ...
    if getgenv then
        if getgenv().LIGHTING_AUTOLOAD == nil then
            getgenv().LIGHTING_AUTOLOAD = false
        end
    end

    local autoLoadEnabled = (togglesState["autoLoadKick"] == true) or (getgenv and getgenv().LIGHTING_AUTOLOAD == true) or false
    local kickConn = nil

    local alOn = autoLoadEnabled
    local alRow = New("Frame", {
        Size=UDim2.new(0.93,0,0,ITEM_H), BackgroundColor3=C.DarkRow,
        BackgroundTransparency=0.3, ZIndex=7, Parent=Pages["Combat"],
    })
    New("UICorner", {CornerRadius=UDim.new(0,8), Parent=alRow})
    New("UIStroke", {Color=C.Gray, Thickness=1, Transparency=0.6, Parent=alRow})
    New("TextLabel", {
        Text="Auto Load for Kick", Size=UDim2.new(0.62,0,1,0), Position=UDim2.new(0.04,0,0,0),
        TextColor3=Color3.new(1,1,1), Font=Enum.Font.GothamBold, TextSize=12,
        BackgroundTransparency=1, TextXAlignment="Left", ZIndex=8, Parent=alRow,
    })
    local alBg = New("Frame", {
        Size=UDim2.new(0,38,0,18), Position=UDim2.new(1,-44,0.5,-9),
        BackgroundColor3=alOn and Color3.fromRGB(200, 30, 30) or C.TogOff, ZIndex=8, Parent=alRow,
    })
    New("UICorner", {CornerRadius=UDim.new(1,0), Parent=alBg})
    local alDot = New("Frame", {
        Size=UDim2.new(0,14,0,14),
        Position=alOn and UDim2.new(0.55,0,0.1,0) or UDim2.new(0.08,0,0.1,0),
        BackgroundColor3=Color3.new(1,1,1), ZIndex=9, Parent=alBg,
    })
    New("UICorner", {CornerRadius=UDim.new(1,0), Parent=alDot})
    local autoLoadRow = alRow
    table.insert(TAB_ROWS["Combat"], alRow)
    rebuildPage("Combat")
    ...
    local function setAutoLoad(v)
        autoLoadEnabled = v
        alOn = v
        if getgenv then getgenv().LIGHTING_AUTOLOAD = v end
        togglesState["autoLoadKick"] = v
        salvarToggles()
        TweenService:Create(alBg,  TweenInfo.new(0.18), {BackgroundColor3 = v and Color3.fromRGB(200, 30, 30) or C.TogOff}):Play()
        TweenService:Create(alDot, TweenInfo.new(0.18), {Position = v and UDim2.new(0.55,0,0.1,0) or UDim2.new(0.08,0,0.1,0)}):Play()
        if kickConn then kickConn:Disconnect(); kickConn = nil end
        if v then
            kickConn = Players.PlayerRemoving:Connect(function(player)
                if player == lp then doReload() end
            end)
        end
    end
...
do
    ...
    local function makeNumRow(labelText, getValue, setValue, minV, maxV, decimals)
        ...
        box.FocusLost:Connect(applyValue)
        return row, box
    end

    makeSettingsTitle("â–¸ DUEL SPEEDS")
    ...
    makeSettingsTitle("â–¸ AIM SPEEDS")
    ...
    makeNumRow("Aim Bat Speed",
        function()
            if _abDadosRef then return _abDadosRef.speed or AimSpeed end
            return AimSpeed
        end,
        function(v)
            if _abDadosRef then
                _abDadosRef.speed = v
                cfgSet("autoBatConfig", _abDadosRef)
            end
        end, 1, 300, true)

    makeSettingsTitle("â–¸ GRAB / STEAL")
    ...
    makeKbRow("Hold Duel",
        function() return _holdKB.key end,
        function(kc)
            _holdKB.key = kc
            local cfg = cfgGet("holdDuelPos") or {}
            cfg.keybind = kc.Name; cfgSet("holdDuelPos", cfg)
        end,
        nil, "holdDuelPos", "keybind")
...
local function showToast(message)
    ...
    toast.BackgroundColor3 = Color3.fromRGB(35, 8, 8)
...
;(function()
    ...
    UIS.InputEnded:Connect(function(i)
        ...
        if MainFrame.Visible then
            ...
            TweenService:Create(mStroke, TweenInfo.new(0.2), {Color = C.LightGray}):Play()
            ...
            TweenService:Create(mStroke, TweenInfo.new(0.18), {Color = C.White}):Play()
        end
    end)
end)()
...
task.defer(function()
    task.wait(0.4)
    local keys = {
        "lock", "autoDuel", "speedCustomizer",
        "meleeAimbot", "infJump", "antiRagdoll", "noAnim", "playerEsp",
        "float", "fov", "optimizer", "xrayBase",
        "autoSteal", "hitboxExp", "autoBat", "aimBatHub", "dropBrainrot", "holdDuel",
    }
    _hubRestoring = true
